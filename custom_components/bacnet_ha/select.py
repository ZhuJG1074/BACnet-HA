    ...
    for obj in coordinator.objects:
        ...
        if domain == "select":
            if obj["object_type"] in {
                OBJECT_TYPE_MULTI_STATE_OUTPUT,
                OBJECT_TYPE_MULTI_STATE_VALUE,
            } and obj.get("commandable"):
                entities.append(
                    BACnetSelect(coordinator, obj_key, obj, entry.entry_id)
                )
            elif obj.get("options") and obj.get("commandable"):
                # AnalogValue with custom options (FanSwitch, SystemSwitch)
                entities.append(
                    BACnetSelectWithOptions(coordinator, obj_key, obj, entry.entry_id)
                )
...
class BACnetSelectWithOptions(CommandableBACnetEntity, SelectEntity):
    """BACnet AnalogValue mapped to a Select entity with user-defined options.

    Used for FanSwitch (自动/高风/中风/低风) and SystemSwitch (制冷/制热/送风).
    The BACnet value is the index (0-based) into the options list.
    """
    ...
    def __init__(
        ...
        self._options = obj.get("options", ["选项0", "选项1"])
        self._object_description = obj.get("description", "")
        self._bacnet_object_type = obj["object_type"]
        self._bacnet_instance = obj["instance"]
    ...
    async def async_select_option(self, option: str) -> None:
        """Select a new option and write the index to the BACnet device."""
        try:
            idx = self._options.index(option)
            success = await self._write_value(idx)
            if not success:
                raise HomeAssistantError(
                    f"Failed to set option '{option}' on BACnet object "
                    f"{self._bacnet_object_type}:{self._bacnet_instance}"
        ...
        if value is not None and not self._flag_out_of_service:
            try:
                ...
                self._attr_current_option = (
                    self._options[idx] if 0 <= idx < len(self._options) else None
                )
            except (TypeError, ValueError, IndexError):
                self._attr_current_option = None
        elif not self._flag_out_of_service:
